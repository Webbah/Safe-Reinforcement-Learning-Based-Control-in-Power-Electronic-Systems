...
from experiments.voltage_forming_control_dq.envs.my_randproc import RandProcess
from experiments.voltage_forming_control_dq.envs.random_load import RandomLoad
...
if not cfg['is_dq0']:
    ...
    net = Network.load('experiments/voltage_forming_control_dq/grids/net_p10.yaml')
else:
    # load net using dq0 reference values

    net = Network.load('experiments/voltage_forming_control_dq/grids/net_p10_dq0.yaml')
...
cb.append(partial(gen.reset, initial=False))
...
register(id='vctrl_single_inv_train-v0',
         ...
         kwargs=dict(  # reward_fun=rew.rew_fun,
             ...
             model_params={'lc.resistor1.R': R_filter,
                           ...
                           'r_load.resistor1.R': R_load_fixed,
                           'r_load.resistor2.R': R_load_fixed,
                           'r_load.resistor3.R': R_load_fixed,
                           ...
                           'lc.inductor1.i': lambda t: 0 if t == -1 else None,
                           'lc.inductor2.i': lambda t: 0 if t == -1 else None,
                           'lc.inductor3.i': lambda t: 0 if t == -1 else None,
                           'inverter1.v_DC': lambda t: v_DC if t == -1 else v_DC,
                           #'lc.capacitor1.v': lambda t: np.random.uniform(low=-v_nom, high=v_nom) if t == -1 else None,
                           #'lc.capacitor2.v': lambda t: np.random.uniform(low=-v_nom, high=v_nom) if t == -1 else None,
                           #'lc.capacitor3.v': lambda t: np.random.uniform(low=-v_nom, high=v_nom) if t == -1 else None,
                           #'lc.inductor1.i': lambda t: np.random.uniform(low=-i_nom, high=i_nom) if t == -1 else None,
                           #'lc.inductor2.i': lambda t: np.random.uniform(low=-i_nom, high=i_nom) if t == -1 else None,
                           #'lc.inductor3.i': lambda t: np.random.uniform(low=-i_nom, high=i_nom) if t == -1 else None,
                           },
             net=net,
             #model_path='experiments/voltage_forming_control_dq/grids/grid.P10_R_load.fmu',
...
rand_load_test = RandomLoad(2881, net.ts, gen,
                             load_curve=pd.read_pickle('experiments/voltage_forming_control_dq/envs/R_load_hard_test_case_10_seconds.pkl'))

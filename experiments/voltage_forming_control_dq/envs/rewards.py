...
class Reward:
    def __init__(self, nom, lim, v_DC, gamma, det_run=False, nom_region: float = 1.1, use_gamma_normalization=1,
        ...
        self.i_lim = i_lim
        self.i_nom = i_nom
        self.i_ratio = self.i_nom/self.i_lim
    ...
    def rew_fun_dq0(self, cols: List[str], data: np.ndarray, risk) -> float:
        ...
        if any(np.abs(data[idx[2]]) > 1) or any(np.abs(data[idx[0]]) > 1):
            if self.det_run:
            ...
            # using safeguard the
            rew = np.sum(1 - ((np.abs(vsp_dq0_master - vdq0_master) / (2*self.v_max/self.lim)) ** self.exponent)) / 3 \
                  #* (1 - self.gamma) / 3  # /2

            if any(np.abs(data[idx[0]]) > (self.i_ratio + 0.1)):    # +0.1 to consider meas. noise...
                for i_mess in data[idx[0]]:

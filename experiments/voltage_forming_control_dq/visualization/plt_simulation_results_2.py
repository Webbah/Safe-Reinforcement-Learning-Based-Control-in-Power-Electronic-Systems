...
#study_name = '370_vDC_SL_RLS_Rvar'
study_name = '370_vDC_SL_RLS'

trial = ['1']
episode_list = [ '55']  # , '10', '52', '66', '75']
# episode_list = ['2', '34', '46' ,'60']
terminated_list = ['0', '0', '0', '0']
add_str = ['', '', '_TEST']
RLS = 1
...
interval_list_x = [0.09, 0.102]
interval_list_x = [0.0, 0.05]
...
for tr in range(len(trial)):
    ...
    if episode_list is not None:
        for episode, terminated in zip(episode_list, terminated_list):
            ...
            if plt_R:
                fig, axs = plt.subplots(6, 1)#, figsize=(9, 7))
            else:
            ...
            axs[3].set_ylabel('$u_{\mathrm{abc,RL}}$')
            ...
            if plt_R:
                axs[5].plot(t, episode_data['R_load_training'].to_list(), 'g')
                # axs[4].plot(t, episode_data['safe_guard_active'].to_list(), 'g')
                #axs[4].set_ylabel('SG-active?')
                axs[5].grid()
                if xlim:
                    axs[5].set_xlim(interval_list_x)
                # axs[4].plot(t, episode_data['R_load_training'].to_list(), 'g')
                axs[5].set_ylabel('$R_{\mathrm{Load}}\,/\,\mathrm{\Omega}$')
                axs[5].set_xlabel('$t_{\mathrm{}}\,/\,\mathrm{s}$')
            ...
            if save_results:
                fig.savefig(f'{folder_name}/' + study_name + 'timeseries_abc_end.pgf')
                fig.savefig(f'{folder_name}/' + study_name + 'timeseries_abc_end.png')
                fig.savefig(f'{folder_name}/' + study_name + 'timeseries_abc_end.pdf')
            ...
            if RLS:
                ...
                axs[0, 0].plot(t[1:], episode_data['A11_c'].to_list()[1:], 'b', label='$A_\mathrm{d, RLS}$')
                axs[0, 0].plot(t[1:], A_d2[0, 0, 1:], ':', color='red', label='$A_\mathrm{d}$')
                axs[0, 0].legend(bbox_to_anchor=(0, 1.02, 1, 0.2), loc="lower left", mode="expand", borderaxespad=0,
                              ncol=4)
                ...
                axs[0, 0].set_ylabel('$A_{\mathrm{d,11}}$')
                ...
                axs[0, 1].set_ylabel('$A_{\mathrm{d,12}}$')
                ...
                axs[1, 0].set_ylabel('$A_{\mathrm{d,21}}$')
                ...
                axs[1, 1].set_ylabel('$A_{\mathrm{d,22}}$')
                ...
                if save_results:
                    fig.savefig(f'{folder_name}/' + study_name + 'Ac.pgf')
                    fig.savefig(f'{folder_name}/' + study_name + 'Ac.png')
                    fig.savefig(f'{folder_name}/' + study_name + 'Ac.pdf')
